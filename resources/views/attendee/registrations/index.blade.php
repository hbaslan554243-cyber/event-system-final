...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold mb-1">My Registrations</h3>
            <p class="text-muted mb-0">All your event registrations</p>
        </div>
                ...
                <div class="col-md-3 text-end">
                    ...
                    <div class="d-flex gap-2 justify-content-end flex-wrap">
                        @if(in_array($reg->status, ['confirmed', 'attended']))
                            <a href="{{ route('attendee.tickets.show', $reg) }}"
                               class="btn btn-sm btn-primary">
                                <i class="bi bi-ticket me-1"></i>Ticket
                            </a>
                        @endif

                        @if($reg->payment_status === 'pending' && $reg->status === 'pending')
                        ...
                        @if($reg->event->status === 'completed' && $reg->status === 'attended')
                            @php
                                $hasFeedback = \App\Models\Feedback::where('event_id', $reg->event_id)
                        ...
                        @if($reg->status === 'confirmed' && $reg->event->start_date > now())
                            <form method="POST"
                                  action="{{ route('attendee.registrations.cancel', $reg) }}"
    ...
    <div class="card border-0 shadow-sm text-center py-5">
        <div class="text-muted">
            ...
            <p class="small mb-3">Register for an event to see it here.</p>
